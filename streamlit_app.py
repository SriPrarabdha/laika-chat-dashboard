...
API_BASE_URL = "https://chat-dashboard-239264243926.us-central1.run.app/"
API_KEY = str(uuid.uuid5(uuid.NAMESPACE_DNS, "laika_dashboard"))
SECRET_KEY = "manas123"  # Secret key for authentication
...
if not st.session_state.authenticated:
    ...
    <div class='login-container'>
        <div class='login-header'>
            ...
            <p>Please enter your secret key to access the dashboard</p>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.form("login_form"):
        ...
        if submit_button:
            if authenticate(secret_key_input):
                st.success("Authentication successful!")
                st.experimental_rerun()
            else:
                st.error("Invalid secret key. Please try again.")

# Show the main application only if authenticated
else:
    # Sidebar
    st.sidebar.title("Chat Dashboard")
    page = st.sidebar.radio("Navigation", ["Index", "Search"])
